---
title: "ETH MSNA 2024: Data Collection Tracker"
author: "REACH-Initiates"
date: "JUNE 2024"
output:
    html_document:
    toc: true
toc_float:
    collapsed: false
smooth_scroll: false
knit: (function(inputFile, encoding) {rmarkdown::render(inputFile, encoding = encoding, output_file = paste0(dirname(inputFile), '/eth_msna_2024_data_collection_tracker_', format(Sys.Date(), '%Y_%m_%d'),'.html')) })
---
...
# read data
df_tool_data <- readxl::read_excel("../inputs/ETH2403_MSNA_2024_data.xlsx")  |>  
  mutate(uuid = `_uuid`,
         start_date = as_date(start),
         start = as_datetime(start),
         end = as_datetime(end),
         latitude = as.numeric(household_geopoint_latitude),
         longitude = as.numeric(household_geopoint_longitude))
...
df_cl_log <- read_csv(file = "../inputs/combined_checks_eth_msna.csv") |> 
...
for (current_uuid in uuids_chg_response) {
  current_uuid_data <- cl_log_change_response |> 
    ...
    mutate(value = ifelse(name == "enum_id", as.numeric(value), value)) |> 
...
# sample data
df_samples_required <- readxl::read_excel("../support_files/msna_sample_size.xlsx") |> 
    select(admin4, required_samples = tot_sample_size)
...
df_zone_info <- df_choices |> 
  filter(list_name %in% c("l_admin2")) |> 
...
df_zone_to_kebele_info <- df_choices |> 
  filter(list_name %in% c("l_admin4")) |> 
  select(admin2_code = filter_admin1, admin3_code = filter_admin3, code, kebele = label) |> 
  left_join(df_zone_info, by = c("admin2_code" = "code")) |>
  relocate(zone, .after = admin2_code) |>
  left_join(df_woreda_info, by = c("admin3_code" = "code")) |>
  relocate(woreda, .after = admin3_code) |>
  select(-c(admin2_code, admin3_code))
...
df_updated_tool_data |> 
  ...
  left_join(df_zone_to_kebele_info, by = c("admin4" = "code")) |> 
  relocate(c(zone, woreda, kebele), .before = number_of_surveys) |>
  select(-c(zone, admin4)) |> 
  dt_options_fewcols()
```
